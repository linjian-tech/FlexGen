    ...
    for i in range(len(weight_specs)):
        ...
        if not compress:
            ...
            if DUMMY_WEIGHT not in filename:
                weight.load_from_np_file(weight_specs[i][2]) # weight_specs[i][2] represents path
            else:
...
class InputEmbed:
    ...
    def init_weight(self, weight_home, path):
        v, h, dtype = (self.config.vocab_size, self.config.hidden_size, self.config.torch_dtype)
        path = os.path.join(path, "")
        weight_specs = [
            # w_token
            ((v, h), dtype, path + "embed_tokens.weight"),
            # remove w_pos in new transformer architecture
...
class OutputEmbed:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_specs = [
            ...
            # b_ln
            # ((h,), dtype, path + "decoder.layer_norm.bias"), # remove bias
            # w_token
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.opt_output_embed(h, w_ln, w_token, donate,
...
class SelfAttention:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_specs = [
            # w_q
            ((self.num_attention_heads * self.head_dim, h), dtype, path + "self_attn.q_proj.weight"),
            # b_q
            # ((h,), dtype, path + ".q_proj.bias"),
            # w_k
            ((self.num_key_value_heads * self.head_dim, h), dtype, path + "self_attn.k_proj.weight"),
            # b_k
            # ((h,), dtype, path + ".k_proj.bias"),
            # w_v
            ((self.num_key_value_heads * self.head_dim, h), dtype, path + "self_attn.v_proj.weight"),
            # b_v
            # ((h,), dtype, path + ".v_proj.bias"),
            # w_out
            ((h, self.num_attention_heads * self.head_dim), dtype, path + "self_attn.o_proj.weight"),
            # b_out
...
class MLP:
    ...
    def init_weight(self, weight_home, path):
        h, intermediate_size, dtype = (self.config.hidden_size, self.config.intermediate_size, self.config.torch_dtype)
        path = os.path.join(os.path.join(path, f"layers.{self.layer_id}."))
        weight_specs = [
            # wi
    ...
    def load_weight(self, weight_home, weight_read_buf, k):
        wi, wo, w_gate, w_ln = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((wi, donate[1]), (w_gate, donate[2]), (wo, donate[3]),
             (w_ln, donate[4])) = weight_read_buf.pop()
        else:
            ((wi, _),  (w_gate, _), (wo, _),
             (w_ln, _)) = weight_read_buf.val

        h = self.compute.mlp(h, wi, w_gate, wo, w_ln, donate, self.config.rms_norm_eps, self.act_fn)
        hidden.val = h
...
class OptLM:
    ...
    def init_weight(self, j):
        ...
        if not os.path.exists(check_path) and DUMMY_WEIGHT not in check_path:
            download_llama_weights(self.model, self.path)
    ...
    def compute_layer(self, i, j, k):
        ...
        if isinstance(self.layers[j], SelfAttention):
            self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
                                   ...
                                   self.cache_write_buf[j][k], i, k, self.position_embeddings)
        else:
            self.layers[j].forward(self.hidden[i][j][k], self.cache_read_buf[j][k],
            self.weight_read_buf[j], self.attention_mask[k],
            self.cache_write_buf[j][k], i, k)

        if isinstance(self.layers[j], InputEmbed):
            token_ids = self.hidden[i][j][k].val.data
            mask = self.attention_mask[k].val.data

            positions = torch.cumsum(mask, dim=1).int() * mask - 1
            # cut positions if `past_key_values_length` is > 0
            past_key_values_length = mask.shape[1] - token_ids.shape[1]
            positions_ids = positions[:, past_key_values_length:]
            self.position_embeddings = self.rotary_emb(token_ids, positions_ids)
...
def run_flexllmgen(args):
    ...
    model = OptLM(llama_config, env, model_name, args.path, policy)
