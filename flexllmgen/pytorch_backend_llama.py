...
class TorchDevice:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        ...
        shape = (prompt_len + gen_len - 1, gpu_batch_size * num_head, hidden_size // config.num_attention_heads)
...
class TorchDisk:
    ...
    def init_cache_one_gpu_batch(self, config, task, policy):
        num_head, hidden_size, prompt_len, gen_len, gpu_batch_size = (
            config.num_key_value_heads, config.input_dim, task.prompt_len, task.gen_len,
            policy.gpu_batch_size)
        shape = (prompt_len + gen_len - 1, gpu_batch_size * num_head, config.num_attention_heads)
